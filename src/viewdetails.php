 <?php
    ...
    if (isset($_SESSION['fname'])) {
        ...
        $query = "Select count(serial_no) from participate where pid=" . $pid;
        $query1 = "select first_name ,last_name from user_data 
                    where 
                    serial_no in (Select serial_no from participate where pid=" . $pid . ")";
         ...
         <script>
             ...
             window.addEventListener('load', async () => {
                 ...
                 const startstopvoting = async (i) => {
                     ...
                     await contract.methods.startstopVoting(i).send({
                         from: account
                 ...
                 if (message[4]) {
                     document.getElementById("change").innerHTML = "<h3> voting is open  </h3><button id='changeStatus'   class='btn btn-success' onclick='myfun()'>Stop voting</button>";
                     console.log("fhgj");
                 } else {
             ...
             async function myfun() {
                 ...
                 var pid = <?php echo $pid;
                            ?>;
                 if (acc === account) {
                     contract.methods.startstopVoting(pid).send({
                         from: account
                     }).then(function(result) {

                         window.location.href = "viewdetails.php?pid=" + p;
