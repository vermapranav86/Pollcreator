  ...
  <script src="web3.min.js"></script> -->
        ...
        <script>
            window.addEventListener('load', async () => {
                ...
                var polldetails = await pvcontractMessage(<?php echo $pid; ?>);

                if (polldetails[4]) {
                    $("#question").html(polldetails[1]);
                    var ops = polldetails[4].map(web3.utils.hexToUtf8)
                    console.log(ops);
                ...
                $("input[type='button']").click(async function() {
                    ...
                    if (acc === account) {
                        contract.methods.votePrvt(parseInt(pid), vote).send({
                            from: account
                        }).then(async function(receipt) {
                            window.location.href = "insertpvparticipant.php?pid=" + pid;
